...
void or_0C(InputReader* ir, RegisterBank* rb, Memory* mem){
    ...
    uint8_t imm8 = InstructionUtils::getImm8(ir);

    rb->set("AL", al|imm8);

    // Set flags - No overflow possible, so no need to cast to long
    InstructionUtils::setFlagGroup(al|imm8, 8, rb); 
}
...
void or_0D(InputReader* ir, RegisterBank* rb, Memory* mem){
    ...
    uint32_t imm32 = InstructionUtils::getImm32(ir);

    rb->set("EAX", eax|imm32);

    // Set flags
    InstructionUtils::setFlagGroup(eax|imm32, 32, rb);
}
